...
namespace PESpy.Controls
{
    internal class TreeListView : ListView
    {
        ...
        {
#if WINFORMS
            //Right clicking into a ListView as soon as we run ReDbg can cause flickering. DoubleBuffering appears to solve this
            DoubleBuffered = true;
        ...
        {
            _hyperLinkFont = new Font(Font, FontStyle.Underline);

            _hFont = Font.ToHfont();
            _hHyperLinkFont = _hyperLinkFont.ToHfont();
        ...
        protected override unsafe void WndProc(ref System.Windows.Forms.Message m)
        {
            var lParam = m.LParam;
#else
        protected unsafe void WndProc(ref Message m)
        {
            var lParam = m.lParam;
#endif
            switch ((WM) m.Msg)
            {
                case WM.WM_REFLECT_NOTIFY:
                    if (WmReflectNotify((NMHDR*) (nint) lParam))
                    {
                        m.Result = (nint) CDRF.CDRF_SKIPDEFAULT;
                        return;
                    }
                ...
                case WM.WM_LBUTTONDBLCLK:
                    ...
                    if (!WmMouseDown((short) LOWORD((uint) lParam), (short) HIWORD((uint) lParam)))
                        base.WndProc(ref m);
            ...
            in Rectangle bounds)
        {
            ...
            var image = subItem.ImageIndex == -1 ? null : SmallImageList.Images[subItem.ImageIndex];
        ...
        private void DrawSelectedSubItem(
            ...
            Rectangle bounds)
        {
            //We only support full row select
            Debug.Assert(FullRowSelect);
        ...
        internal static unsafe void DrawString(
            ...
            out int imageX)
        {
            Gdi32.SetTextColor(hdc, new COLORREF(color));
        ...
        private void DrawImage(HDC hdc, Image? image, int imageX, in Rectangle bounds)
        {
            if (image == null)
                return;
            ...
            g.DrawImage(image, new Rectangle(imageX, bounds.Top + 1 + (bounds.Height - image.Height) / 2, image.Width, image.Height));
        }
        ...
        private Color GetSelectedForegroundColor(ListViewSubItem subItem)
        {
            if (Focused)
        ...
        private Color GetSelectedBackgroundColor(ListViewSubItem subItem)
        {
            if (Focused)
        ...
        #endregion
        #region WmReflectNotify

        private unsafe bool WmReflectNotify(NMHDR* nmhdr)
        {
            if (nmhdr->code != (int) NM.NM_CUSTOMDRAW)
                return false;

            var customDraw = (NMLVCUSTOMDRAW*) nmhdr;

            //Based on WinForms
            if (customDraw->nmcd.dwDrawStage == (NMCUSTOMDRAW_DRAW_STAGE.CDDS_SUBITEM | NMCUSTOMDRAW_DRAW_STAGE.CDDS_ITEMPREPAINT))
            {
                var hdc = customDraw->nmcd.hdc;

                var itemIndex = (int) customDraw->nmcd.dwItemSpec;

                //Protect against spurious draw notifications
                var itemBounds = GetItemRectOrEmpty(itemIndex);

                if (!ClientRectangle.IntersectsWith(itemBounds))
                    return true; //Nobody should paint this

                var listViewItem = (TreeViewItem) Items[itemIndex];

                /* Apparently the native ListView sends an event for every column even
                 * if no item actually exists for that column. WinForms suppresses such
                 * notifications. We could potentially take advantage of this fact to avoid
                 * allocating empty subitems for our Meaning column, but then we'd have the
                 * problem of trying to figure out what the bounds of that subitem is */
                if (customDraw->iSubItem >= listViewItem.SubItems.Count)
                    return true; //Nobody should paint this

                var subItemBounds = GetSubItemRect(itemIndex, customDraw->iSubItem);

                //The first item in a ListView is not a subitem, but the whole item itself.
                //Thus, whenwe're processing the first item we need to restrict its width
                //to that of its header
                if (customDraw->iSubItem == 0 && listViewItem.SubItems.Count > 1)
                    subItemBounds.Width = Columns[0].Width;

                if (!ClientRectangle.IntersectsWith(subItemBounds))
                    return true; //Nobody should paint this

                var subItem = listViewItem.SubItems[customDraw->iSubItem];

                DrawSubItem(hdc, itemIndex, customDraw->iSubItem, listViewItem, (TreeViewItem.TreeViewSubItem) subItem, subItemBounds);
        ...
        private unsafe Rectangle GetItemRectOrEmpty(int index)
        {
        ...
        private unsafe Rectangle GetSubItemRect(int itemIndex, int subItemIndex)
        {
        ...
        private bool WmMouseDown(int x, int y)
        {
            //If the user clicks on a hyperlink, suppress default processing so
            //WinForms doesn't call DefWndProc and select the line. Otherwise,
            //relay the message to the base WndProc

            //Hit test to see if they clicked on the expander
            var hitItem = HitTest(new Point(x, y));

            //Clicking around randomly can cause you to not have hit any SubItem
            if (hitItem.SubItem == null)
                return false;

            var subItem = (TreeViewItem.TreeViewSubItem) hitItem.SubItem;

            if (subItem.IsLink)
            //The SubItem has the actual column we hit
            var bounds = subItem.Bounds;

            var treeViewItem = (TreeViewItem) hitItem.Item;

            var subItemIndex = hitItem.Item.SubItems.IndexOf(hitItem.SubItem);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            _currentHyperLink = subItem;
            _currentHyperLink.IsLinkActive = true;
            Cursor = Cursors.Hand;
        ...
        private void ProcessRightKey()
        {
            ...
            {
                //If we're expanded, move the selection to our first child

                SelectedItems.Clear();

                var child = firstSelectedItem.Nodes[0];
                child.Selected = true;
                FocusedItem = child; //This is important, as when you do up arrow key it seems to relate to this item. If you did a bunch of right arrows from the top, up arrow key would mean "go up from the top" but FocusedItem is already the top item
                child.EnsureVisible();
            }
        ...
        public unsafe void AutoSizeColumns(IList<TreeViewItem> items)
        {
            ...
            /* We've got a bit of an issue when it comes to the scrollbar. If we use the total available width,
             * and a scrollbar is needed, if our last column header uses all of the available space, it's going to
             * end up partially under the scrollbar. When the scrollbar is shown, a resize event is raised.
             * If we then try and correctly adjust the width of the last column to no longer be under the scrollbar,
             * there seems to be a bug in the ListView because when it does nc paint it ends up showing a horizontal
             * scrollbar even though it's not needed. If you then try and scroll using the scrollbar it realizes
             * that actually the scrollbar is not needed and disappears...so why did it show it in the first place
             * then!
             * 
             * You can prevent nc paint from showing this phantom scrollbar by not having the last line go
             * underneath the scrollbar in the first place. Then, in the resize event, we tell the last column
             * to resize to fill all of the available space (using width -2) and this seems to make everything
             * work the way we want, even when no scrollbar will be displayed */
            ...
            for (var i = 0; i < columnHeaders.Count; i++)
            {
                ...
                else
                {
                    ...
                    columnHeaders[i].Width = Math.Max(lastColumnWidth, Math.Min(columnWidths[i], maxWidth));
                }
            }
        }

...
namespace PESpy
{
    ...
    public struct VisualSection
    {
        ...
        public int Width;
        public (int startAddress, int pixelAddress, IntPtr pViewByte)[] Data; //startAddress is the address that the entity starts. pixelAddress is the address of the pixel (which might be part of the body)
        public (int startPixel, int endPixel, int directoryIndex)[]? Directories;

        public int GetBestPixel(int address, int lo, int hi)
        {
            var data = Data;

...
namespace PESpy.Controls
{
    ...
    internal struct ViewMapPainter
    {
        ...
        private ViewMap _viewMap; //Provides access to the various GDI handles we need in order to paint
        ...
        /// </summary>
        public unsafe void RenderBaseLayer()
        {
        ...
        private unsafe void DrawSection(int start, bool isHighlighted, in SectionAccessor sectionAccessor, in VisualSection visualSection, FileAccessor fileAccessor)
        {
            ...
            if (visualSection.Directories != null)
            {
                ...
                for (var j = 0; j < visualSection.Directories.Length; j++)
                {
                    var item = visualSection.Directories[j];

                    var leftPixel = start + item.startPixel;

                    Gdi32.MoveToEx(hMemDC, leftPixel, _yOffset - 20);
                    Gdi32.LineTo(hMemDC, leftPixel, _yOffset - 10);

                    if (item.endPixel != item.startPixel)
                    {
                        var rightPixel = start + item.endPixel;

                        Gdi32.MoveToEx(hMemDC, rightPixel, _yOffset - 20);
                        Gdi32.LineTo(hMemDC, rightPixel, _yOffset - 10);

                        Gdi32.MoveToEx(hMemDC, leftPixel, _yOffset - 20);
                        Gdi32.LineTo(hMemDC, rightPixel, _yOffset - 20);
                    }

                    var directory = ((PEFileAccessor) fileAccessor).DataDirectories[item.directoryIndex];

                    var oldColor = Gdi32.SetTextColor(hMemDC, new COLORREF(0, 0, 0));

                    int minRight1 = xPos;
                    DrawText(hMemDC, directory.Name, leftPixel, _yOffset - 40 - (item.directoryIndex * 10), ref minRight1, _viewMap._codeFontHeight);
        ...
        private unsafe HPEN GetUsagePen(ViewByte* pViewByte, bool isHighlighted, FileAccessor fileAccessor, int startAddress)
        {
        ...
        public void Resize(HDC hdc, in RECT clientRect)
        {
            ...
            Gdi32.SelectObject(hMemDC, _viewMap._hCodeFont);
            Gdi32.SetBkMode(hMemDC, BACKGROUND_MODE.TRANSPARENT);
            Gdi32.SetTextColor(hMemDC, new COLORREF(255, 255, 255));

...
namespace PESpy.UI
{
    internal struct SingletonWriter
    {
        ...
        internal SingletonWriter(TreeListView listView, FileAccessor fileAccessor, bool refresh)
        {
            _listView = listView;
        ...
        public unsafe void Compute(int offset, ViewKind viewKind)
        {
            if (_refresh)
            {
                //Just clear items; we're maintaining the headers
            ...
            ref PooledList<TreeViewItem> pendingListViewItems)
        {
            ...
            if (structView.Kind == ViewKind.ImageDosHeader)
            {
                if (!_refresh)
                {
                    if (_listView.Columns.IndexOfText(ColumnNames.Description) == -1)
                    {
                        //The field names are too obscure; add a description
                        _listView.Columns.Add(ColumnNames.Description);
                    }
                }
            ...
            ref PooledList<TreeViewItem> pendingListViewItems)
        {
            ...
            if (!_refresh)
            {
                if (_listView.Columns.IndexOfText(ColumnNames.Meaning) == -1)
                    _listView.Columns.Add(ColumnNames.Meaning);
            }

...
{
    /// <summary>
    /// Old subsection module information (32-bit / <see cref="LEFile"/>)<para/>
    /// "smd" in the Microsoft C 6.0 Developer's Toolkit Reference; "oldsmd" in cvexefmt.h
    /// </summary>
    public readonly struct smd32 : IValue, IViewable
    {
        private const int SegInfoOffset = 0;
        private const int ovlNbrOffset = nsg32.StructSize;
        private const int iLibOffset = nsg32.StructSize + 2;
        private const int cSegOffset = nsg32.StructSize + 4;
        private const int reservedOffset = nsg32.StructSize + 5;
        private const int nameOffset = nsg32.StructSize + 6;
        private int arnsgOffset => FixedStructSize + name.Length + 1;

        /// <summary>
        /// Describes first segment in module
        /// </summary>
        public nsg32 SegInfo => new nsg32(chunk);
        ...
        public nsg32[] arnsg
        {
            get
            {
                ...
                if (cSeg == 0)
                    return Array.Empty<nsg32>();

                var results = new nsg32[cSeg - 1];
                ...
                for (var i = 0; i < results.Length; i++)
                {
                    ...
                    results[i] = new nsg32(chunk.Slice(read));
                    read += nsg32.StructSize;
                }
        ...
        internal smd32(in MemoryChunk chunk)
        {
            this.chunk = chunk;
        }
        ...
        IView? IViewable.WriteStruct(ViewWriter writer) =>
            writer.NewStruct(Strings.smd32, this, ViewKind.smd32, StructSize);

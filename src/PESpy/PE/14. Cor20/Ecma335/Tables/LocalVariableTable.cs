...
namespace PESpy.Ecma335
{
    public sealed class LocalVariableTable : Table<LocalVariableRow>
    {
        ...
        internal LocalVariableTable(int numRows, int stringIndexSize, Lazy<StringHeap?> stringHeap, in MemoryChunk tableChunk) : base(numRows)
        {
            ...
            AttributesOffset = 0;
            IndexOffset = AttributesOffset + sizeof(int);
            NameOffset = IndexOffset + sizeof(uint);
        ...
        public LocalVariableAttributes GetAttributes(LocalVariableIndex index)
        {
            ...
            return (LocalVariableAttributes) tableChunk.PeekUInt32(rowOffset + AttributesOffset);
        }

        public uint GetIndex(LocalVariableIndex index)
        {
            var rowOffset = (index.RowId - 1) * RowSize;
            return tableChunk.PeekUInt32(rowOffset + IndexOffset);
        }

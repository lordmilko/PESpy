...
namespace PESpy
{
    public readonly unsafe partial struct UnwindCode
    {
        ...
        public readonly struct AllocSmall : IViewable
        {
            ...
            /// <summary>
            /// The size of the allocation is the operation info field * 8 + 8, allowing allocations from 8 to 128 bytes.<para/>
            /// This property returns the scaled size (pre-multiplied by 8 plus 8)
            /// </summary>
            public int Size => OpInfo * 8 + 8;

            internal const int StructSize =
                sizeof(byte) + //CodeOffset
                sizeof(byte);  //UnwindOp + OpInfo

            public AllocSmall(byte* value)
            {
                this.value = value;
            }

            public static implicit operator byte*(AllocSmall value) => value.value;
            ...
            void IViewable.WriteChild(int index, ref StructWriter structWriter)
            {
                switch (index)
                {
                    ...
                    case 2:
                        //No need to unscale; we have a separate property for the Size

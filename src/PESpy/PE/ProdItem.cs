...
{
    /// <summary>
    /// Represents the <see cref="PRODITEM"/> structure which describes an entry in the Rich Header.<para/>
    /// This structure is undocumented.
    /// </summary>
    ...
    public readonly struct ProdItem : IValue, IViewable //Stored in an array, so can be a struct
    {
        ...
        internal ProdItem(int start, int bufferPos, Span<byte> bytes)
        {
            ...
            ProductId = GetProductIdDisplayName(ProdId);
            VisualStudioVersion = GetVisualStudioVersion(ProdId);
        }

        private static string GetProductIdDisplayName(int prodId)
        {
            //These names can be found in msobj*.lib under the Visual Studio installation directory
            //Based on https://github.com/kirschju/richheader/blob/master/prodids.py (see ThirdPartyNotices.txt)
        ...
        private static string? GetVisualStudioVersion(int prodId)
        {
            return prodId switch
            {
                >= 0x0106 and < 0x010a + 1 => "Visual Studio 2017 14.01+",
                >= 0x00fd and < 0x0106 => "Visual Studio 2015 14.00",
                ...
                >= 0x0019 and < 0x0045 + 1 => "Visual Studio 2002 07.00",

                0xA or 0xB or 0xD or 0x15 or 0x16 => "Visual Studio 6.0 06.00",

                0x2 or 0x6 or 0xC or 0xE => "Visual Studio 97 05.00",

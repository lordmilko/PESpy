...
namespace PESpy
{
    ...
    public readonly struct SymStoreKey
    {
        ...
        public static SymStoreKey FromPE(string name, uint timeDateStamp, int sizeOfImage)
        {
        ...
        public static SymStoreKey FromRSDSI(RSDSI rsds)
        {
            //SymSrv seems to use uppercase GUIDs, and apparently certain symbol servers only support uppercase

            var name = Path.GetFileName(rsds.Path.ToString()); //symsrv doesn't seem to modify the case. It could potentially be the full path of the original file location
            var index = $"{name}/{rsds.Guid.ToString("N").ToUpperInvariant()}{rsds.Age:X}/{name}";

            return new SymStoreKey(index, SymStoreKeyKind.PDB);
        }

        public static SymStoreKey FromNB10(NB10I nb10)
        {
            var name = Path.GetFileName(nb10.Path.ToString());

            var index = $"{name}/{nb10.PdbSignature:X}{nb10.Age:X}/{name}";

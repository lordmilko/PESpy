...
namespace PESpy
{
    ...
    internal class ByteMatcher
    {
        ...
        static ByteMatcher()
        {
            ...
            //Using a trie to match might be fast, but it just uses _way_ too much memory
            //tree = ByteSequenceTreeNode.BuildTree(sequences.Keys.ToArray());
        }

        internal static bool TryMatch(PEFile peFile, int virtualOffset, out ByteMatchKind kind)
        {
            ...
            {
                var match = GetMatch(valueChunk);

                if (match != null)
                {
                    kind = sequences[match.Value.Sequence];
        ...
        private static ByteMatch? GetMatch(MemoryChunk chunk)
        {
            //Using a trie just uses way too much memory usage. We don't have that many patterns, so we'll try and use a linear search
            var maxBytes = chunk.Remaining;

            foreach (var kv in sequences)
            {
                var sequence = kv.Key;

                var maxLength = Math.Min(sequence.Length, maxBytes);

                var span = chunk.PeekNativeSpan<byte>(0, maxLength);

                for (var i = 0; i < maxLength; i++)
                {
                    if (!sequence.HasByte(i, span[i]))
                        break;

                    if (i == sequence.Length - 1)
                        return new ByteMatch(i, sequence);
                }
            }

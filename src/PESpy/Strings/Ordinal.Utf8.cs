...
namespace System.Globalization
{
    internal static partial class Ordinal
    {
        ...
        private static bool EqualsIgnoreCaseUtf8_Scalar(ref byte charA, int lengthA, ref byte charB, int lengthB)
        {
        ...
        private static bool EqualsStringIgnoreCaseUtf8(ref byte strA, int lengthA, ref byte strB, int lengthB)
        {
            ...
            while ((length != 0) && (charA <= maxChar) && (charB <= maxChar))
            {
                ...
                if (charA == charB ||
                    ((charA | 0x20) == (charB | 0x20) && IsAsciiLetter((char)charA)))
                {
                    length--;
        ...
        internal static bool StartsWithStringIgnoreCaseUtf8(ref byte source, int sourceLength, ref byte prefix, int prefixLength)
        {
            ...
            {
                // Ordinal equals or lowercase equals if the result ends up in the a-z range
                if (source == prefix || ((source | 0x20) == (prefix | 0x20) && IsAsciiLetter((char)source)))
                {
                    length--;

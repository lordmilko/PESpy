...
{
    class PDBFileViewWriterHelper : IViewWriterHelper
    {
        public FileKind FileKind => pdbFile.Kind;
        ...
        public PDBFile PDBFile => pdbFile;

        private PDBFile pdbFile;

        internal PDBFileViewWriterHelper(PDBFile pdbFile)
        {
            ...
            DirectoryInfo[] pages)
        {
            ...
            try
            {
                for (var i = 0; i < pdbFile.NumPages; i++)
                {
                    ...
                    if (pageToSIMap.TryGetValue((uint) i, out var match))
                    {
                        //string name;
                    ...
                    pages[i] = new DirectoryInfo(fullNameInfo, (uint) i * pdbFile.PageSize, pdbFile.PageSize);
                }
            }
            finally
            {

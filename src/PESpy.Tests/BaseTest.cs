...
#pragma warning restore CS8509
        ...
        protected TVerifier GetStruct<TSelector, TVerifier>(out Stream fs)
        {
            ...
            object rawValue = name switch
            {
                ...
                "VsVersionInfo.StringFileInfo"       => GetFile(WellKnownTestModule.ntdll, out fs).ResourceDirectory!.EnumerateResources<VsVersionInfo>().First().Children[0],
                "VsVersionInfo.StringTable"          => ((VsVersionInfo.StringFileInfo) GetFile(WellKnownTestModule.ntdll, out fs).ResourceDirectory!.EnumerateResources<VsVersionInfo>().First().Children[0]).Children[0],
                ...
                "ScopeTable.ScopeRecord"             => ((ScopeTable) GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[28].UnwindData.Value.ExceptionData).Records[0],
                ...
                nameof(TryBlockMapEntry)             => (((RVA<FuncInfo>) GetFile(WellKnownTestModule.AuthExt, out fs).ExceptionTable[74].UnwindData.Value.ExceptionData).Value).TryBlockMap.Value[0],
                nameof(HandlerType)                  => (((RVA<FuncInfo>) GetFile(WellKnownTestModule.AuthExt, out fs).ExceptionTable[74].UnwindData.Value.ExceptionData).Value).TryBlockMap.Value[0].HandlerArray.Value[0],
                nameof(TypeDescriptor)               => ((RVA<FuncInfoV1>) GetFile(WellKnownTestModule._7z, out fs).ExceptionTable[500].UnwindData.Value.ExceptionData).Value.TryBlockMap.Value[0].HandlerArray.Value[0].Type.Value,
                nameof(UnwindMapEntry)               => (((RVA<FuncInfo>) GetFile(WellKnownTestModule.AuthExt, out fs).ExceptionTable[74].UnwindData.Value.ExceptionData).Value).UnwindMap.Value[0],
                nameof(IptoStateMapEntry)            => (((RVA<FuncInfo>) GetFile(WellKnownTestModule.AuthExt, out fs).ExceptionTable[74].UnwindData.Value.ExceptionData).Value).IPToStateMap.Value[0],
                ...
                "UnwindCode.PushNonVolatile"         => (UnwindCode.PushNonVolatile)    GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[0].UnwindData.Value.UnwindCode[1],
                "UnwindCode.AllocLarge"              => (UnwindCode.AllocLarge)         GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[0].UnwindData.Value.UnwindCode[0],
                "UnwindCode.AllocSmall"              => (UnwindCode.AllocSmall)         GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[2].UnwindData.Value.UnwindCode[0],
                "UnwindCode.SetFpReg"                => (UnwindCode.SetFpReg)           GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[291].UnwindData.Value.UnwindCode[3],
                "UnwindCode.SaveNonVolatile"         => (UnwindCode.SaveNonVolatile)    GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[1].UnwindData.Value.UnwindCode[0],
                //"UnwindCode.SaveNonVolatileFar"      => (UnwindCode.SaveNonVolatileFar) GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[].UnwindData.Value.UnwindCode[],
                "UnwindCode.Epilog"                  => (UnwindCode.Epilog)             GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[1800].UnwindData.Value.UnwindCode[0],
                "UnwindCode.SaveXmm128"              => (UnwindCode.SaveXmm128)         GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[948].UnwindData.Value.UnwindCode[0],
                //"UnwindCode.SaveXmm128Far"           => (UnwindCode.SaveXmm128Far)      GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[].UnwindData.Value.UnwindCode[],
                "UnwindCode.PushMachFrame"           => (UnwindCode.PushMachFrame)      GetFile(WellKnownTestModule.ntdll, out fs).ExceptionTable[2602].UnwindData.Value.UnwindCode[19],
                ...
                nameof(GuardAddressTakenIatEntryTable) => GetSampleFile(Sample.SingleFileApp_EXE, out fs).LoadConfigTable?.GuardAddressTakenIatEntryTable.Value,
                "GuardAddressTakenIatEntryTable.Entry" => GetSampleFile(Sample.SingleFileApp_EXE, out fs).LoadConfigTable?.GuardAddressTakenIatEntryTable.Value[0],
                ...
                nameof(GuardLongJumpTargetTable)       => GetFile(WellKnownTestModule.ntoskrnl, out fs).LoadConfigTable?.GuardLongJumpTargetTable.Value,
                "GuardLongJumpTargetTable.Entry"       => GetFile(WellKnownTestModule.ntoskrnl, out fs).LoadConfigTable?.GuardLongJumpTargetTable.Value[0],
                ...
                nameof(ImageBoundForwarderRef)     => GetFile(WellKnownTestModule.mfc40u, out fs).BoundImportTable?[0].Refs[0],
                ...
                nameof(ImageDelayLoadDescriptor) => GetFile(WellKnownTestModule.coreclr, out fs).DelayImportTable[0],
                ...
                nameof(ImageCorILMethodSectEH)   => GetFile(WellKnownTestModule.mscorlib, out fs).ILMethods[22].EHSections[0],
                nameof(ImageCorILMethodSect)     => GetFile(WellKnownTestModule.mscorlib, out fs).ILMethods[22].EHSections[0].Sect,
                nameof(ImageCorILMethodSectEHClause) => GetFile(WellKnownTestModule.mscorlib, out fs).ILMethods[22].EHSections[0].Clauses[0],
                nameof(StorageStream)            => GetFile(WellKnownTestModule.mscorlib, out fs).EcmaMetadata.Header.StreamHeaders[0],

                nameof(EcmaMetadata)             => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata,
                nameof(CompressedModelHeap)      => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata.CompressedModelHeap,
                nameof(CompressedModelHeader)    => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata.CompressedModelHeap.Header,
                nameof(StringHeap)               => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata.StringHeap,
                nameof(BlobHeap)                 => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata.BlobHeap,
                nameof(GuidHeap)                 => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata.GuidHeap,
                nameof(UserStringHeap)           => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata.UserStringHeap,

                nameof(BlobEntry)                => GetSampleFile(Sample.Framework_EXE, out fs).EcmaMetadata.BlobHeap.GetBlob(1),
                nameof(UserString)               => GetFile(WellKnownTestModule.mscorlib, out fs).EcmaMetadata.UserStringHeap.GetString(1),

                nameof(ReadyToRunHeader)         => GetSampleFile(Sample.R2R_DLL, out fs).ReadyToRunHeader,
                nameof(ReadyToRunCoreHeader)     => GetSampleFile(Sample.R2R_DLL, out fs).ReadyToRunHeader.CoreHeader,
                nameof(ReadyToRunSection)        => GetSampleFile(Sample.R2R_DLL, out fs).ReadyToRunHeader.CoreHeader.Sections[0],

                //0: CompilerIdentifier
                nameof(ReadyToRunImportSection)  => ((ReadyToRunImportSection[]) GetSampleFile(Sample.R2R_DLL, out fs).ReadyToRunHeader.CoreHeader.Sections[1].Data)[1],
                //2: RuntimeFunctions
                ...
                "Bundle.HeaderFixed"             => (GetSampleFile(Sample.SingleFileApp_EXE, out fs).AppHostSignature).BundleHeaderOffset.Value.Header,
                "Bundle.HeaderFixedV2"           => (GetSampleFile(Sample.SingleFileApp_EXE, out fs).AppHostSignature).BundleHeaderOffset.Value.AdditionalContext,
                "Bundle.Location"                => (GetSampleFile(Sample.SingleFileApp_EXE, out fs).AppHostSignature).BundleHeaderOffset.Value.AdditionalContext.DepsJsonLocation,
                "Bundle.FileEntry"               => (GetSampleFile(Sample.SingleFileApp_EXE, out fs).AppHostSignature).BundleHeaderOffset.Value.Files[0],
                "Bundle.FileEntryFixed"          => (GetSampleFile(Sample.SingleFileApp_EXE, out fs).AppHostSignature).BundleHeaderOffset.Value.Files[0].Header,
                nameof(BundleEncodedString)      => (GetSampleFile(Sample.SingleFileApp_EXE, out fs).AppHostSignature).BundleHeaderOffset.Value.BundleID,

                nameof(ClrEngineMetrics)         => GetSampleFile(Sample.SingleFileApp_EXE, out fs).ClrEngineMetrics,
